...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (commandName === 'ticket') {
            ...
            // الصورة الرئيسية (التذكرة)
            const mainBannerPath = getAssetPath('banner-main.png');
            // صورة القوانين
            const rulesBannerPath = getAssetPath('banner-rules.png');

            try {
                // إرسال صورة التذكرة الرئيسية
                if (mainBannerPath) {
                    const mainAttachment = new AttachmentBuilder(mainBannerPath, { name: 'banner-main.png' });
                    await interaction.channel.send({ files: [mainAttachment] });
                }

                // إرسال صورة القوانين تحتها
                if (rulesBannerPath) {
                    const rulesAttachment = new AttachmentBuilder(rulesBannerPath, { name: 'banner-rules.png' });
                    await interaction.channel.send({ files: [rulesAttachment] });
                }
        ...
        if (commandName === 'setup-logs') {
            ...
            await interaction.reply({
                embeds: [new EmbedBuilder()
                    .setDescription('✅ تم تعيين <#' + targetChannel.id + '> كقناة سجلات.')
                    .setColor(0x57F287)],
                flags: 64
            });
            return;
        ...
        if (commandName === 'setup-staff') {
            ...
            await interaction.reply({
                embeds: [new EmbedBuilder()
                    .setDescription('✅ تم تعيين <@&' + role.id + '> كرتبة إدارة.')
                    .setColor(0x57F287)],
                flags: 64
            });
            return;
        ...
        if (commandName === 'ticket-stats') {
            const cfg = loadGuildConfig(guildId);
            const sortedClaims = Object.entries(cfg.claims || {})
                .sort((a, b) => b[1] - a[1]).slice(0, 5);
            let claimsValue = 'لا توجد بيانات بعد';
            if (sortedClaims.length > 0) {
                claimsValue = sortedClaims
                    .map((e, i) => (i + 1) + '. <@' + e[0] + '> — **' + e[1] + '** استلامات')
                    .join('\n');
            }
            await interaction.reply({
                embeds: [new EmbedBuilder()
        ...
        if (commandName === 'reset-stats') {
            ...
            await interaction.reply({
                embeds: [new EmbedBuilder()
                    .setDescription('✅ تم إعادة تعيين الإحصائيات.')
                    .setColor(0xED4245)
                    .setTimestamp()],
                flags: 64
            });
        ...
        if (commandName === 'close-ticket') {
            ...
            if (!isAdmin && !hasStaffRole) {
                await interaction.reply({ content: 'هذا الأمر للإدارة فقط.', flags: 64 });
                return;
            }
            const claimantId = cfg.channelClaimants && cfg.channelClaimants[channel.id];
            if (claimantId && !isAdmin && closer.id !== claimantId) {
                await interaction.reply({
                    content: '❌ فقط عضو الإدارة الذي استلم التذكرة (<@' + claimantId + '>) أو الإدارة يمكنهم الإغلاق.',
                    flags: 64
                });
                return;
            }
        ...
        if (commandName === 'remove-user') {
            ...
            if (targetUser.id === ownerId) {
                await interaction.reply({ content: '❌ لا يمكنك إزالة صاحب التذكرة.', flags: 64 });
                return;
            }
            if (targetUser.id === claimantId) {
                await interaction.reply({ content: '❌ لا يمكنك إزالة عضو الإدارة المستلمة.', flags: 64 });
                return;
            }
            if (!channel.permissionOverwrites.cache.get(targetUser.id)) {
                await interaction.reply({ content: '❌ هذا المستخدم لم يُضف إلى التذكرة.', flags: 64 });
                return;
            }
        ...
        if (commandName === 'add-user') {
            ...
            if (!isAdmin && !hasStaffRole && !isTicketOwner) {
                await interaction.reply({ content: 'هذا الأمر للإدارة وصاحب التذكرة فقط.', flags: 64 });
                return;
            }
            const targetUser = interaction.options.getUser('user');
            const targetMember = await interaction.guild.members.fetch(targetUser.id).catch(() => null);
            if (!targetMember) {
                await interaction.reply({ content: '❌ العضو غير موجود في السيرفر.', flags: 64 });
                return;
            }
        ...
        if (commandName === 'rename-ticket') {
            ...
            if (!isAdmin && !hasStaffRole && requester.id !== claimantId) {
                await interaction.reply({ content: '❌ فقط الإدارة المستلمة أو الإدارة يمكنهم إعادة التسمية.', flags: 64 });
                return;
            }
            const newName = interaction.options.getString('name')
                .toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9\-]/g, '').slice(0, 100);
            if (!newName) {
                await interaction.reply({ content: '❌ اسم غير صالح.', flags: 64 });
                return;
            }
            ...
            await interaction.reply({
                embeds: [new EmbedBuilder()
                    .setDescription('✏️ تم التغيير من `' + oldName + '` إلى `' + newName + '`.')
                    .setColor(0xE8B923)
                    .setTimestamp()]
            });
            return;
        ...
        if (commandName === 'staff-help') {
            ...
            const hasCategoryRole = cfg.categoryRoles &&
                Object.values(cfg.categoryRoles).some(rid => member.roles.cache.has(rid));
            ...
            await interaction.reply({
                embeds: [new EmbedBuilder()
                    .setTitle('📋 أوامر الإدارة')
                    .setDescription('الأوامر المتاحة داخل التذاكر:')
                    .addFields(
                    ...
                    .setColor(0xE8B923)
                    .setFooter({ text: 'استلام التذكرة أولاً قبل أي إجراء' })
                    .setTimestamp()],
                flags: 64
            });
            return;
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'ticket-menu') {
        ...
        try {
            ...
            guild.roles.cache
                .filter(r => r.permissions.has(PermissionFlagsBits.Administrator) && r.id !== activeRoleId)
                .forEach(r => {
                    permissionOverwrites.push({
                        id: r.id,
                        allow: [
            ...
            // بنر القسم المختار داخل التذكرة
            ...
            try {
                await interaction.editReply({
                    content: '❌ خطأ: `' + (err && err.message ? err.message : String(err)) + '`'
                });
            } catch {}
        }
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'setup-category-select') {
        ...
        await interaction.update({
            embeds: [new EmbedBuilder()
                ...
                .setDescription('القسم: **' + categoryLabels[selectedCat] + '**\n\nاختر الرتبة:')
                .setColor(0xE8B923)],
    ...
    if (interaction.isRoleSelectMenu() && interaction.customId.startsWith('setup-category-role:')) {
        ...
        await interaction.update({
            embeds: [new EmbedBuilder()
                .setTitle('✅ تم التخصيص')
                .setDescription('القسم **' + categoryLabels[selectedCat] + '** → <@&' + selectedRole.id + '>')
                .setColor(0x57F287)
                .setTimestamp()],
            components: []
        });
    ...
    if (interaction.isButton() && interaction.customId.startsWith('claim-ticket:')) {
        ...
        const hasStaffRole =
            (cfg.staffRoleId && claimant.roles.cache.has(cfg.staffRoleId)) ||
        ...
        try {
            ...
            const activeStaffRoleId =
                (channelCat && claimCfg.categoryRoles && claimCfg.categoryRoles[channelCat]) ||
                claimCfg.staffRoleId;
            ...
            if (claimBannerPath && claimBannerFile) {
                claimReplyPayload.files = [new AttachmentBuilder(claimBannerPath, { name: claimBannerFile })];
            }
    ...
    if (interaction.isButton() && interaction.customId.startsWith('close-ticket:')) {
        ...
        const hasStaffRole =
            (cfg.staffRoleId && closer.roles.cache.has(cfg.staffRoleId)) ||
        ...
        if (claimantId && !isAdmin && closer.id !== claimantId) {
            try {
                await interaction.reply({
                    content: '❌ فقط عضو الإدارة المستلمة (<@' + claimantId + '>) أو الإدارة يمكنهم الإغلاق.',
                    flags: 64
                });
            } catch {}
            return;
        ...
        if (closeBannerPath && closeBannerFile) {
            closeReplyPayload.files = [new AttachmentBuilder(closeBannerPath, { name: closeBannerFile })];
        }
        await interaction.editReply(closeReplyPayload);

        setTimeout(async () => {
            await channel.delete('أُغلقت بواسطة ' + closer.user.tag).catch(() => null);
        }, 5000);
        return;
    }
});
