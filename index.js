...
async function loadGuildConfig(guildId) {
    const doc = await db.collection('guilds').findOne({ guildId });
    if (!doc) return defaultGuildConfig();
    const { _id, guildId: _g, ...data } = doc;
    return Object.assign(defaultGuildConfig(), data);
}
...
async function incrementGuildTicket(guildId) {
    const result = await db.collection('guilds').findOneAndUpdate(
        { guildId },
        { $inc: { ticketCount: 1, totalTickets: 1 } },
...
const categoryBanners = {
    'purchase': 'banner-purchase.png',
    'refund':   'banner-refund.png',
...
const commands = [
    ...
    new SlashCommandBuilder()
        ...
        .addChannelOption(opt =>
            opt.setName('channel').setDescription('القناة المستهدفة')
...
client.once('ready', async () => {
    console.log('Bot جاهز: ' + client.user.tag);
    try {
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (commandName === 'ticket') {
            ...
            const mainBannerPath  = getAssetPath('banner-main.png');
            const rulesBannerPath = getAssetPath('banner-rules.png');

            try {
                if (mainBannerPath) {
                    await interaction.channel.send({ files: [new AttachmentBuilder(mainBannerPath, { name: 'banner-main.png' })] });
                }
                if (rulesBannerPath) {
                    await interaction.channel.send({ files: [new AttachmentBuilder(rulesBannerPath, { name: 'banner-rules.png' })] });
                }
        ...
        if (commandName === 'setup-logs') {
            ...
            await interaction.reply({
                embeds: [new EmbedBuilder().setDescription('✅ تم تعيين <#' + targetChannel.id + '> كقناة سجلات.').setColor(0x57F287)],
                flags: 64
            });
            return;
        }
        ...
        if (commandName === 'setup-staff') {
            ...
            await saveGuildConfig(guildId, cfg);
            await interaction.reply({
                embeds: [new EmbedBuilder().setDescription('✅ تم تعيين <@&' + role.id + '> كرتبة إدارة.').setColor(0x57F287)],
                flags: 64
            });
            return;
        }
        ...
        if (commandName === 'ticket-stats') {
            ...
            const claimsValue = sortedClaims.length > 0
                ? sortedClaims.map((e, i) => (i + 1) + '. <@' + e[0] + '> — **' + e[1] + '** استلامات').join('\n')
                : 'لا توجد بيانات بعد';
            await interaction.reply({
        ...
        if (commandName === 'reset-stats') {
            ...
            await saveGuildConfig(guildId, cfg);
            await interaction.reply({
                embeds: [new EmbedBuilder().setDescription('✅ تم إعادة تعيين الإحصائيات.').setColor(0xED4245).setTimestamp()],
                flags: 64
            });
            return;
        }
        ...
        if (commandName === 'close-ticket') {
            ...
            const closeChannelCat = cfg.channelCategories?.[channel.id];
            ...
            if (cfg.logsChannelId) {
                ...
                if (logsChannel) {
                    await logsChannel.send({
                        embeds: [new EmbedBuilder().setTitle('📋 سجل التذكرة #' + ticketNum)
                            .addFields(
                                { name: '👤 الصاحب', value: ownerId ? '<@' + ownerId + '>' : 'غير معروف', inline: true },
                                { name: '🔒 أُغلقت بواسطة', value: '<@' + closer.id + '>', inline: true },
            ...
            if (cfg.channelCategories) delete cfg.channelCategories[channel.id];
            await saveGuildConfig(guildId, cfg);
        ...
        if (commandName === 'add-user') {
            ...
            const isAdmin = requester.permissions.has(PermissionFlagsBits.Administrator);
            const hasStaffRole = cfg.staffRoleId && requester.roles.cache.has(cfg.staffRoleId);
            const isTicketOwner = cfg.channelOwners?.[channel.id] === requester.id;
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'ticket-menu') {
        ...
        try {
            const cfg = await loadGuildConfig(guildId);
            const ticketNumber = await incrementGuildTicket(guildId);
            const guild = interaction.guild;
            const member = interaction.member;
            const selectedValue = interaction.values[0];

            const categoryLabels = { 'purchase': 'شراء', 'refund': 'استرجاع', 'support': 'دعم فني' };
            const categoryEmojis = { 'purchase': '🛒', 'refund': '↩️', 'support': '🛠️' };

            const channelName = 'ticket-' + ticketNumber;
            const categoryRoleId = cfg.categoryRoles?.[selectedValue];
            ...
            const ticketEmbed = new EmbedBuilder()
                ...
                .setDescription('أهلاً بك! يرجى شرح طلبك وسيتولى أحد الإدارة مساعدتك قريباً.')
                .setColor(0xE8B923).setTimestamp();
            if (bannerPath && bannerFile) ticketEmbed.setImage('attachment://' + bannerFile);

            const claimButton = new ButtonBuilder().setCustomId('claim-ticket:' + ticketNumber + ':' + member.id).setLabel('استلام التذكرة').setStyle(ButtonStyle.Primary).setEmoji('👤');
            const closeButton = new ButtonBuilder().setCustomId('close-ticket:' + ticketNumber + ':' + member.id).setLabel('إغلاق التذكرة').setStyle(ButtonStyle.Danger).setEmoji('🔒');
            ...
            if (bannerPath && bannerFile) openMsgPayload.files = [new AttachmentBuilder(bannerPath, { name: bannerFile })];
            await channel.send(openMsgPayload);

            // حفظ بيانات القناة في قاعدة البيانات
            const freshCfg = await loadGuildConfig(guildId);
            if (!freshCfg.channelOwners) freshCfg.channelOwners = {};
            freshCfg.channelOwners[channel.id] = member.id;
            if (!freshCfg.channelCategories) freshCfg.channelCategories = {};
            freshCfg.channelCategories[channel.id] = selectedValue;
            await saveGuildConfig(guildId, freshCfg);

            await interaction.editReply({ content: '✅ تم فتح تذكرتك: <#' + channel.id + '>' });
        } catch (err) {
            console.error('[ticket-create error]', err);
            try { await interaction.editReply({ content: '❌ خطأ: `' + (err?.message || String(err)) + '`' }); } catch {}
        }
        return;
    }

    // ── setup-category step 1 ────────────────────────────────────────────────
    ...
    if (interaction.isButton() && interaction.customId.startsWith('claim-ticket:')) {
        ...
        const channelCat = cfg.channelCategories?.[channel.id];
        const catRoleId = channelCat && cfg.categoryRoles?.[channelCat];
        ...
        try {
            ...
            cfg.claims[claimant.id] = (cfg.claims[claimant.id] || 0) + 1;
            cfg.channelClaimants = cfg.channelClaimants || {};
            cfg.channelClaimants[channel.id] = claimant.id;
            await saveGuildConfig(guildId, cfg);
            ...
            const activeStaffRoleId = (channelCat && cfg.categoryRoles?.[channelCat]) || cfg.staffRoleId;
            ...
            const claimEmbed = new EmbedBuilder().setDescription('👤 تم الاستلام بواسطة <@' + claimant.id + '>.\nسيتولى طلبك.').setColor(0xE8B923).setTimestamp();
    ...
    if (interaction.isButton() && interaction.customId.startsWith('close-ticket:')) {
        ...
        const isAdmin = closer.permissions.has(PermissionFlagsBits.Administrator);
        const closeChannelCat = cfg.channelCategories?.[channel.id];
        const closeCatRoleId = closeChannelCat && cfg.categoryRoles?.[closeChannelCat];
        ...
        if (!isAdmin && !hasStaffRole) { try { await interaction.reply({ content: 'فقط الإدارة يمكنها إغلاق التذكرة.', flags: 64 }); } catch {} return; }
        if (claimantId && !isAdmin && closer.id !== claimantId) { try { await interaction.reply({ content: '❌ فقط عضو الإدارة المستلمة (<@' + claimantId + '>) أو الإدارة يمكنهم الإغلاق.', flags: 64 }); } catch {} return; }
        ...
        if (cfg.logsChannelId) {
            ...
            if (logsChannel) {
                await logsChannel.send({
                    embeds: [new EmbedBuilder().setTitle('📋 سجل التذكرة #' + ticketNumber)
                        .addFields(
                            { name: '👤 الصاحب', value: ownerId ? '<@' + ownerId + '>' : 'غير معروف', inline: true },
                            { name: '🔒 أُغلقت بواسطة', value: '<@' + closer.id + '>', inline: true },
